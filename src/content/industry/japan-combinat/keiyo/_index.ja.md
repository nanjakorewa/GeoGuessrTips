...
| 会社 | エチレン能力 | 主な製品 |
| ---- | ---------- | ------- |
| 京葉エチレン（市原・丸善石油化学55%・住友化学45%合弁） | 69万t/年 | エチレン・プロピレン・ブタジエン |
...
| コスモ石油千葉製油所 | 市原市 | 177,000 BD | ガソリン・灯油・軽油・石化原料ナフサ |
...
<strong>京葉地区 エチレン製造能力（合計210万t/年）</strong>
<canvas id="pie-ethylene" style="max-height:280px;margin-top:8px;"></canvas>
</div>
<div style="background:#fff;border-radius:10px;box-shadow:0 1px 8px rgba(0,0,0,.07);padding:20px;text-align:center;">
<strong>京葉地区 原油処理能力（合計71.3万BD）</strong>
<canvas id="pie-refinery" style="max-height:280px;margin-top:8px;"></canvas>
</div>
</div>

※ エチレン能力は石油化学工業協会資料に基づく{{% cite "jpca2025" %}}。原油処理能力は石油連盟公表値{{% cite "paj_refinery" %}}。
    ...
    tooltip: {
      callbacks: {
        label: function(ctx) {
          ...
          return ' ' + ctx.label + ': ' + ctx.parsed + '万' + (ctx.chart.canvas.id === 'pie-ethylene' ? 't' : 'BD') + ' (' + (ctx.parsed/t*100).toFixed(1) + '%)';
        }
      }
    }
  }
...
new Chart(document.getElementById('pie-ethylene'), {
  type:'doughnut',
  data:{
    labels:['京葉エチレン（住友化学・三菱ケミカル等）','三井化学','丸善石油化学','出光興産'],
    datasets:[{data:[69, 55.5, 48, 37.4],
      backgroundColor:['#1565c0','#2e7d32','#e65100','#6a1b9a']}]
  }, options:pieOpts
});

new Chart(document.getElementById('pie-refinery'), {
  type:'doughnut',
  data:{
    labels:['出光興産（22万BD）','ENEOS（17.5万BD）','コスモ石油（17.5万BD）','富士石油（14.3万BD）'],
    datasets:[{data:[22, 17.5, 17.5, 14.3],
      backgroundColor:['#6a1b9a','#1565c0','#e65100','#2e7d32']}]
  }, options:pieOpts
});
</script>
