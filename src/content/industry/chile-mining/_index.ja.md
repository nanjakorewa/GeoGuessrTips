...
<tr><td>BHP Group</td><td>BHP</td><td>豪メルボルン本社、世界最大級の鉱業メジャー。チリではEscondida鉱山（57.5%、世界最大の銅鉱山）、Pampa Norte（Spence/Cerro Colorado）、Resolution（米）等の銅事業を展開。グローバルでは鉄鉱石（豪Pilbara・WAIO）、石炭（豪QLD原料炭）、カリ（加Jansen建設中）も保有。</td><td>{{% corplink "https://www.bhp.com/investors" %}}</td><td>{{% dividend "other" "BHP" %}}</td></tr>
<tr><td>Rio Tinto</td><td>RIO</td><td>英ロンドン／豪メルボルン本社の世界2位の鉱業メジャー。チリではBHPとのEscondida合弁に30%出資。グローバルでは鉄鉱石（豪Pilbara）、銅（米Kennecott・モンゴルOyu Tolgoi 66%）、アルミニウム（加Sauguenay）、リチウム（セルビアJadar・アルゼンチンRincon）を展開。</td><td>{{% corplink "https://www.riotinto.com/en/invest" %}}</td><td>{{% dividend "other" "RIO" %}}</td></tr>
<tr><td>Freeport-McMoRan</td><td>FCX</td><td>米アリゾナ州フェニックス本社、世界最大級の上場銅企業。インドネシアのGrasberg鉱山（金・銅）、米国アリゾナ・ニューメキシコのMorenci/Bagdad/Sierrita、ペルーのCerro Verde（53.6%）が主力。チリではEl Abra鉱山（51%、Codelcoが49%）を運営。モリブデンも北米で大手。</td><td>{{% corplink "https://investors.fcx.com/" %}}</td><td>{{% dividend "other" "FCX" %}}</td></tr>
<tr><td>Southern Copper</td><td>SCCO</td><td>メキシコ財閥Grupo Méxicoの銅子会社、米アリゾナ／メキシコシティ本社。ペルーのToquepala・Cuajone・Ilo製錬所、メキシコのBuenavista del Cobre・La Caridadを運営し、Mexico・ペルー中心の銅メジャー。チリでは現状主要操業はないが、確認埋蔵量ベースで世界有数の銅企業。</td><td>{{% corplink "https://www.southerncoppercorp.com/investor-center/" %}}</td><td>{{% dividend "other" "SCCO" %}}</td></tr>
<tr><td>Glencore</td><td>GLEN.L</td><td>スイス・ツーク本社、世界最大級の鉱業・コモディティトレーダー（旧Marc Rich & Co）。チリでAnglo Americanとの50/50合弁でCollahuasi銅鉱山（残12%は三井物産/JX金属）、Lomas Bayas銅鉱山を運営。グローバルではコンゴ民主共和国Katanga・Mutandaの銅コバルト、亜鉛、ニッケル、石炭、アルミ等を展開。</td><td>{{% corplink "https://www.glencore.com/investors" %}}</td><td>{{% dividend "other" "GLEN.L" %}}</td></tr>
<tr><td>Anglo American</td><td>AAL.L</td><td>英ロンドン本社、1917年南アで創業した歴史ある鉱業メジャー。チリではLos Bronces（50.1%、三菱商事20.4%・Codelco-三井物産JV 29.5%）、Collahuasi（44%）、El Soldado銅鉱山を運営。グローバルでは鉄鉱石（ブラジルMinas-Rio）、プラチナ（南ア）、ダイヤモンド（De Beers、売却中）、銅（ペルーQuellaveco）を展開。</td><td>{{% corplink "https://www.angloamerican.com/investors" %}}</td><td>{{% dividend "other" "AAL.L" %}}</td></tr>
<tr><td>Teck Resources</td><td>TECK</td><td>カナダ・バンクーバー本社、2024年に石炭事業（Elk Valley Resources）をGlencoreに売却完了し「純粋銅企業」への転換を進める。チリではQuebrada Blanca QB2（60%、住友金属鉱山25%・住友商事5%・Codelco残り）銅鉱山、Carmen de Andacollo、Zafranalプロジェクトを展開。カナダ・米国・ペルーでも銅・亜鉛を保有。</td><td>{{% corplink "https://www.teck.com/investors/" %}}</td><td>{{% dividend "other" "TECK" %}}</td></tr>
<tr><td>SQM</td><td>SQM</td><td>Sociedad Química y Minera de Chile、チリ・サンティアゴ本社。Salar de Atacamaでのかん水型リチウム（世界最大級）、ヨウ素（世界シェア約3分の1）、硝石系特殊肥料（K/Na/Caを含むSPN）、カリ・工業化学品の5事業。2024年にCodelcoとのJV協定でアタカマ操業をCodelco 50%＋SQM 50%（2025-2060年）に移行予定。</td><td>{{% corplink "https://ir.sqm.com/" %}}</td><td>{{% dividend "other" "SQM" %}}</td></tr>
<tr><td>Albemarle</td><td>ALB</td><td>米ノースカロライナ州シャーロット本社、世界最大級のリチウム化学品メーカー。チリのSalar de Atacama（CORFO契約2043年まで）、米ネバダ州Silver Peak（米国唯一の操業中リチウム鉱）、豪Greenbushes（49%、Tianqi+IGOとのJV）スポジュメン鉱から世界の電池グレード水酸化リチウム/炭酸リチウムを供給。臭素事業（Bromine）も主力。</td><td>{{% corplink "https://investors.albemarle.com/" %}}</td><td>{{% dividend "other" "ALB" %}}</td></tr>
<tr><td>Antofagasta plc</td><td>ANTO.L</td><td>英ロンドン上場（FTSE100）、チリのルクシック家（Quiñenco経由）が支配する銅メジャー。チリ国内でLos Pelambres（60%、残り40%はJX金属/丸紅）、Centinela（70%、丸紅30%）、Antucoya、Zaldívarの4銅鉱山を運営。銅精鉱・銅カソードに加え、金・モリブデンも副産物で生産。</td><td>{{% corplink "https://www.antofagasta.co.uk/investors/" %}}</td><td>{{% dividend "other" "ANTO.L" %}}</td></tr>
<tr><td>Lundin Mining</td><td>LUN.TO</td><td>カナダ・トロント本社、スウェーデン系Lundin財閥の中核鉱業会社。チリでCaserones銅鉱山（2023年JX金属/三井物産から買収、51%所有・残り49%もJX保有）、Candelaria（80%、住友金属鉱山20%）銅鉱山を運営。米Eagle（ニッケル）、ブラジルChapada（銅金）、ポルトガルNeves-Corvo等を保有。</td><td>{{% corplink "https://lundinmining.com/investors/" %}}</td><td>{{% dividend "other" "LUN.TO" %}}</td></tr>
<tr><td>KGHM</td><td>KGH.WA</td><td>ポーランド・ルビン本社、欧州最大の銅・銀メーカー（KGHM Polska Miedź）。チリではSierra Gorda銅・モリブデン鉱山を55%（住友金属鉱山31.5%・住友商事13.5%）で運営。本国ポーランドでは世界最大級の銅-銀ベルト「Foresudetic Monocline」を採鉱し、Głogów/Legnica製錬所で電気銅を生産。</td><td>{{% corplink "https://kghm.com/en/investors" %}}</td><td>{{% dividend "other" "KGH.WA" %}}</td></tr>
</tbody></table>
</div>
