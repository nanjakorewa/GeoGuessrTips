...
日本の焼き物は、地域ごとに異なる土・釉薬・焼成方法から多彩な様式が生まれ、産地ごとに個性を持っています。中世から続く**日本六古窯**（常滑・瀬戸・越前・信楽・丹波・備前）はその起点であり{{% cite "rokkoyo" %}}、17世紀以降の磁器技術の発展により有田・九谷・清水など新たな産地が加わりました。
...
| 分類 | 原料 | 焼成温度 | 透光性 | 代表産地 |
| ---- | ---- | -------- | ------ | -------- |
| **陶器**（土もの） | 陶土（地域の粘土） | 1,100〜1,250℃ | なし | 萩・益子・唐津・笠間・京焼（一部） |
| **炻器**（せっき） | 陶土（高温焼き締め） | 1,200〜1,300℃ | わずか | 備前・常滑・越前・丹波・信楽・万古 |
...
| **越前焼** | 福井県越前町 | 日本六古窯。灰釉の自然な風合い。壺・甕の名産地 | 炻器（焼き締め） |

### 東海

| 産地 | 所在地 | 特徴 | 分類 |
| ---- | ------ | ---- | ---- |
| **美濃焼** | 岐阜県土岐市・多治見市ほか | 和洋飲食器の国内シェア約50〜60%を占める最大産地{{% cite "meti2022" %}}。陶器から磁器まで幅広い器種を工業的に量産できる点が強みで、志野・織部などの伝統様式から業務用食器まで対応 | 陶器・磁器 |
...
| 産地 | 所在地 | 特徴 | 分類 |
| ---- | ------ | ---- | ---- |
| **信楽焼** | 滋賀県甲賀市信楽町 | 日本六古窯。たぬきの置物と自然釉・焦げ肌 | 炻器・陶器 |
| **丹波焼（立杭焼）** | 兵庫県丹波篠山市 | 日本六古窯。蹴ろくろで成形。素朴な灰釉 | 炻器（焼き締め） |
...
| 産地 | 所在地 | 特徴 | 分類 |
| ---- | ------ | ---- | ---- |
| **備前焼** | 岡山県備前市 | 日本六古窯。釉薬なし・焼き締め。素地に土の表情 | 炻器 |
...
日本の焼き物産地で培われた窯業技術は、近代以降に**ファインセラミックス（精密陶磁器）**へと進化し、電子部品・半導体パッケージ・環境装置といったハイテク分野を生み出しました。特に愛知県の瀬戸・常滑・名古屋地区は、伝統窯業の集積が近代的セラミックス産業へ転換した典型例です。
...
窯業産地の系譜とは独立に、セラミックスの電気特性（絶縁性・圧電性・磁性）に着目して創業した企業群も存在します。これらは伝統的な窯業からの転換ではなく、セラミックス材料の科学的研究を起点としています。

- **京セラ**（1959年創業）：稲盛和夫が京都で「京都セラミック」として設立{{% cite "kyocera_ir" %}}。ファインセラミックスから電子部品・太陽電池・半導体パッケージへと多角化
- **村田製作所**（1944年創業）：チタン酸バリウムの焼結技術からセラミックコンデンサ（MLCC）を事業化{{% cite "murata_history" %}}。MLCCで世界シェア約40%
- **TDK**（1935年創業）：磁性セラミックスである**フェライト**の工業化に世界で初めて成功{{% cite "tdk_history" %}}。電子部品・蓄電池へ展開
- **イビデン**（1912年創業）：揖斐川の水力発電によるカーバイド製造からスタートし、セラミック技術を蓄積{{% cite "ibiden_ir" %}}。現在はICパッケージ基板（有機基板が主力）とディーゼル排ガス浄化フィルター（DPF）の二本柱

### 産地転換型 — 窯業産地から生まれた現代企業

こちらは伝統的な窯業産地の技術・人材が直接的に近代産業へ転換したケースです。

- **LIXIL**（旧INAX）：常滑焼の伝統を持つ伊奈製陶（1924年設立）が衛生陶器・タイルに展開{{% cite "lixil_ir" %}}。現在はTOSTEMと統合し住宅設備大手に
- **MARUWA**（1946年創業）：瀬戸の窯業家・神戸家が電子用セラミック基板に転換{{% cite "maruwa_history" %}}。パワー半導体向けセラミック基板で高シェア
...
<td>村田製作所</td>
<td>{{% minkabu 6981 %}}</td>
<td>チタン酸バリウム系セラミックスの焼結技術を基盤に創業。積層セラミックコンデンサ（MLCC）で世界シェア約40%の圧倒的首位{{% cite "murata_history" %}}。</td>
